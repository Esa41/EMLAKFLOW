...
export function SettingsForm({
  ...
  const set = <K extends keyof TenantSettings>(k: K, val: TenantSettings[K]) => {
    setV((s) => ({ ...s, [k]: val }));
    setSaved(false);
  };
    ...
    <div className="max-w-3xl space-y-6">
      {!isOwner && (
        <p className="rounded-xl bg-amber-50 px-4 py-2.5 text-sm text-amber-700">
          Ayarları yalnız ofis sahibi (OWNER) değiştirebilir — görüntüleme modundasın.
        </p>
      )}
      ...
      <section className={sectionCls}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <input className={inputCls} value={v.name} disabled={!isOwner}
              onChange={(e) => set("name", e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>Telefon</label>
            <input className={inputCls} value={v.phone} disabled={!isOwner}
              onChange={(e) => set("phone", e.target.value)} placeholder="+90 312 …" />
          </div>
          <div>
          ...
          <div>
            ...
            <input className={inputCls} value={v.city} disabled={!isOwner}
              onChange={(e) => set("city", e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>İlçe</label>
            <input className={inputCls} value={v.district} disabled={!isOwner}
      ...
      <section className={sectionCls}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <input type="number" step="0.1" min={0} max={20} className={inputCls}
              value={v.commissionRate} disabled={!isOwner}
          ...
          <div>
            ...
            <input type="number" min={0} max={100} className={inputCls}
              value={v.agentSharePct} disabled={!isOwner}
        ...
        <p className="mt-3 rounded-xl bg-brand-50 px-4 py-2.5 text-xs text-brand-700">
          ...
          {((10_000_000 * (Number(v.commissionRate) || 0)) / 100).toLocaleString("tr-TR")}
          {" — "}danışman ₺
          {(((10_000_000 * (Number(v.commissionRate) || 0)) / 100) * (agentPct / 100)).toLocaleString("tr-TR")}
          {" / "}ofis ₺
          {(((10_000_000 * (Number(v.commissionRate) || 0)) / 100) * ((100 - agentPct) / 100)).toLocaleString("tr-TR")}
        </p>
      </section>
      ...
      <section className={sectionCls}>
        ...
        <div className="mt-4">
          ...
          />
          <p className="mt-1 text-xs text-ink/45">
            Vitrinin en üstünde, başlığın altında görünür. Boş bırakılırsa standart metin kullanılır.
          </p>
        </div>
        ...
        <div className="mt-4">
          ...
          />
          <p className="mt-1 text-xs text-ink/45">
            İlan detayındaki "WhatsApp'tan yaz" butonu bu numaraya gider; boşsa ofis telefonu kullanılır.
          </p>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="flex gap-2">
            <input className={`${inputCls} font-mono text-xs`} value={showcaseUrl} readOnly />
            <button
            ...
            >
              {copiedShowcase ? <Check size={16} className="text-brand-700" /> : <Copy size={16} />}
            </button>
          </div>
          <p className="mt-1 text-xs text-ink/45">
            Bu linki Instagram biyografine, Google işletme profiline ve kartvizite koy.
          </p>
        </div>
        ...
        <div className="mt-6 border-t border-ink/10 pt-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input className={inputCls} value={v.aboutTitle} disabled={!isOwner} maxLength={90}
            ...
            <div>
              ...
              <textarea className={`${inputCls} min-h-24`} value={v.aboutText} disabled={!isOwner}
                maxLength={800} onChange={(e) => set("aboutText", e.target.value)}
                placeholder="Ofisinizin hikâyesi, çalışma prensibiniz…" />
            </div>
            <div>
              <label className={labelCls}>Vizyon cümlesi</label>
              <input className={inputCls} value={v.visionText} disabled={!isOwner} maxLength={200}
                onChange={(e) => set("visionText", e.target.value)}
                placeholder="Tırnak içinde, tek çarpıcı cümle." />
            </div>
            <div>
              <label className={labelCls}>İstatistik plakaları (en fazla 3)</label>
              <div className="space-y-2">
                {v.aboutStats.map((st, i) => (
                  <div key={i} className="grid grid-cols-[100px_1fr] gap-2">
                    <input className={inputCls} value={st.value} disabled={!isOwner} maxLength={12}
                    ...
                    <input className={inputCls} value={st.label} disabled={!isOwner} maxLength={24}
                      placeholder={["Yıl", "Tamamlanan satış", "Tavsiye oranı"][i] ?? "Etiket"}
                      onChange={(e) => {
                        const next = v.aboutStats.map((x, j) =>
            ...
            <label className="flex items-center justify-between rounded-lg bg-ink/[0.04] px-4 py-3">
              ...
              <input type="checkbox" checked={v.showTeam} disabled={!isOwner}
      ...
      <section className={sectionCls}>
        ...
        <div className="space-y-3">
          {(
            ...
            <label key={key} className="flex items-center justify-between rounded-xl bg-ink/[0.04] px-4 py-3">
        ...
        <div className="mt-4">
          ...
          <div className="flex gap-2">
            <input className={`${inputCls} font-mono text-xs`} value={feedUrl} readOnly />
            <button
              onClick={copyFeed}
          ...
          <p className="mt-1 text-xs text-ink/45">
            Bu adresi portal panelindeki "XML ile ilan aktarımı" alanına yapıştıracaksın.
            Feed çıkışı bir sonraki güncellemeyle aktifleşecek; açık portallardaki
            YAYINDA durumundaki ilanlar otomatik aktarılacak.
          </p>
        </div>
      </section>
