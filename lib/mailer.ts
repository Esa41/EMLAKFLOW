...
export async function sendPasswordResetEmail(to: string, link: string) {
  ...
  <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;color:#17201c">
    ...
    <p style="margin:28px 0">
      <a href="${link}" style="background:#1d1d1f;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 24px;border-radius:10px;display:inline-block">
        Yeni şifre belirle
      </a>
    </p>
    <p style="font-size:12px;line-height:1.6;color:#8a938e">
      Bu isteği siz yapmadıysanız bu e-postayı yok sayın — şifreniz değişmez.
      Buton çalışmazsa bağlantı: <br><a href="${link}" style="color:#1d1d1f">${link}</a>
    </p>
  </div>`;
